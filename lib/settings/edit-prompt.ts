...
export async function editPrompt(
    ...
    const sysPromptString = createSysPrompt(promptRole,
                                            ...
                                            promptFirstMessage, 
                                            promptRules,
                                            persona && !isDefaultPrompt);

    if (chatbotId === 0) {
        await insertChatbot(persona.persona_id, promptRole, promptTask, sysPromptString, isDefaultPrompt);
    } else {
        await updateChatbot(persona.persona_id, promptRole, promptTask, sysPromptString, isDefaultPrompt);
    }
...
//             ]
//         };
//         const sysPromptString = JSON.stringify(sysPrompt);

//         // Perform the necessary operations with the sysPrompt object
//         // ...
//         await insertChatbot(persona.persona_id, promptRole, promptTask, sysPromptString, isDefaultPrompt)
...
//         // Perform the necessary operations with the sysPrompt object
//         // add condition if role is on the default role and isDefaultPrompt is checked
//         await insertChatbot(persona.persona_id, promptRole, promptTask, sysPromptString, isDefaultPrompt)
//     }
