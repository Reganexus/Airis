...
export function AirisChat({ historyConversationId }: AirisChatProps): JSX.Element {
  ...
  useEffect(() => {
    if (historyConversationId) {
      ...
      // 1.
      if (isNaN(numberHistoryConversationId)) { handleInvalidState(); }

      // 2.
      if (status === "unauthenticated") { handleInvalidState(); }
      ...
      if (status === "authenticated" && user != undefined) {
        ...
        const validateUser = async () => {
          ...
          // 3.
          if (data == "wrong uid") { handleInvalidState(); }
          ...
          const getOldChat = async () => {
            try {
              ...
              // 4.
              if (data.error != "") { handleInvalidState(); }
  ...
  const [hoveredMessageIndex, setHoveredMessageIndex] = React.useState<number | null>(null);  // Tracks the index of the message that is currently being hovered over in the UI.
  ...
  const [downloadUrls, setDownloadUrls] = useState<{ [key: string]: string[] }>({});          // Maintains URLs generated for downloading images, organized by a unique key for each set of images.
